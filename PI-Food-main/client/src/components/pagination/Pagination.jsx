...
export default function Pagination({ page, max, setPage }) {
    ...
    const prevPage = () => {
        ...
        setPage(parseInt(page) - 1) //setea el estado. Renderiza 9 recipes previos al estado actual
    };

    const nextPage = () => {
        setInput(parseInt(input + 1));
        setPage(parseInt(page) + 1); //setea el estado. Renderiza 9 recipes siguientes al estado actual
    };
    ...
    return (
        <div className={s.container}>
            <p>{input} of {max}</p>
            <div className={s.numbres}>
                <button
                    className={s.previusButton}
                    onClick={prevPage}
                    disabled={page === 1 || page < 1}> {/* desativa el boton cuando el valor llega a uno  */}
                ...
                <button
                    ...
                    onClick={nextPage}
                    disabled={page === max || page > max}> {/*desativa el boton cuando el valor llega al maximo */}
                    <svg width="20" height="27" viewBox="0 0 20 27" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M0.71469 26.5151C0.930384 26.6273 1.17256 26.6786 1.41522 26.6636C1.65788 26.6485 1.89187 26.5677 2.09207 26.4297L19.426 14.4294C19.6031 14.3066 19.7478 14.1428 19.8478 13.9519C19.9478 13.7611 20 13.5488 20 13.3333C20 13.1178 19.9478 12.9056 19.8478 12.7147C19.7478 12.5238 19.6031 12.36 19.426 12.2373L2.09207 0.236891C1.89203 0.0984771 1.65793 0.0174242 1.41513 0.0025091C1.17233 -0.012406 0.930075 0.039385 0.714596 0.152274C0.499118 0.265163 0.318628 0.434849 0.192669 0.642958C0.06671 0.851067 8.21138e-05 1.08967 1.81114e-08 1.33293V25.3337C-4.00911e-05 25.5771 0.0665396 25.8159 0.192523 26.0241C0.318506 26.2324 0.499086 26.4021 0.71469 26.5151Z" fill="white" />
                    </svg>
                </button>
            </div>
        </div>
